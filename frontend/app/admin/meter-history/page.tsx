...
export default function AdminMeterHistoryPage() {
  const [history, setHistory] = useState<{ timestamp: string; value: number }[]>([]);

  useEffect(() => {
    fetchMeterHistory('admin').then(setHistory);
  }, []);

  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-bold text-brand-800">Fleet meter history</h1>
